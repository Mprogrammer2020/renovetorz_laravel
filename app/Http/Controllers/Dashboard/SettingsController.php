...
class SettingsController extends Controller
{
    ...
    public function SaveGlobalCredits(Request $request){ 
        $request->validate(
            [
                'credit_value' => 'required',
            ],
            [
                'credit_value.required' => 'Please enter Credit value',
            ]
        );
        ...
        if (isset($request->credit_id)) {
            GlobalCredit::where('id', $request->credit_id)->update($data);
        } else {
            $data = GlobalCredit::create($data);
        }
        return response()->json(['status' => true, 'message' => 'Credit saved successfully.']);
    ...
    public function credits(){
        $credits = credits::all();
        // dd($credits);
        return view('panel.admin.settings.index',compact('credits'));
    }

    public function creditSave(Request $request){

        credits::where('user_id', Auth::user()->id)->delete();
        foreach ($request->forms as $credits) {
            $data = [
                'user_id' => Auth::user()->id, 
                'leads_count' => $credits['leads'],
                'credit_value' => $credits['credit_value'],
    ...
    public function creditDelete($id){
        if(!empty($id)){
            ...
            if($credits){
                $credits->delete();
                return response()->json(['status'=> true, 'message' => 'Credits delete successfully']);
            }
        }else{
    ...
    public function KindOfWork(Request $request){
        $kindOfWorkList = kindofwork::orderBy("id")->paginate(10);
        return view('panel.admin.settings.kind_of_work', compact('kindOfWorkList'));
    }
    
    public function SaveKindOfWork(Request $request){
        $data = array(
            "name"=>$request->name,
        ...
        if(!empty($request->work_id)){
            $work = kindofwork::where('id',$request->work_id)->update([
                'name' => $request->name,
                "description"=>$request->description,
            ]);
        }else{
            $work = kindofwork::create($data);
        }

        if($work){
            return response()->json(["status" => true, "success"=> "Work add successfully"]);
    }else{
        return response()->json(["status" => false,"error"=> "Something went wrong"]);
    }
    }

    function DeleteKindOfWork($id){
        $dlt = kindofwork::where('id',$id)->delete();
        if($dlt){
            return response()->json(["status" => true, "success"=> "Deleted successfully"]);
    ...
    public function PropertyType(Request $request){
        $property_type_list = property_type::orderBy("id")->paginate(10);
        // dd($property_type);
        return view('panel.admin.settings.property_type',compact('property_type_list'));
    }
    ...
    public function SavePropertyType(Request $request){
        ...
        if(!empty($request->property_id)){
            $work = property_type::where('id',$request->property_id)->update([
                'name' => $request->name,
                "description"=>$request->description,
            ]);
        }else{
            $work = property_type::create($data);   
        }
    ...
    function DeletePropertyType($id){
        $dlt = property_type::where('id',$id)->delete();
        if($dlt){
            return response()->json(["status" => true, "success"=> "Deleted successfully"]);
