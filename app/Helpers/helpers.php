...
function checkComposerType(){
    $checkComposerType = ComposerType::select('title','status')->where('user_id', Auth::user()->id)->get();
    return $checkComposerType;
}

function workSpaceGroups(){
    $workSpace = workspaceGroupType::with('workspaces')->get();
        
    foreach ($workSpace as $key => $details) {
        $workspaceItems = explode(',', trim($details->workspace_item, '""'));
        $data = OpenAIGenerator::select('id', 'title','slug')->whereIn('id', $workspaceItems)->get();
        $workSpace[$key]->item_data = $data;
    }
    return $workSpace;
 }
