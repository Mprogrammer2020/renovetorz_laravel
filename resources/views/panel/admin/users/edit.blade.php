								...
								<div class="row">
									<div class="col-md-6">
										<div class="mb-3">
											...
											<input type="text" class="form-control" id="name" name="name" value="{{$user->name}}" maxlength="60" onkeydown="return /[a-zA-Z]/i.test(event.key)">
										</div>
									</div>
									<div class="col-md-6">
										<div class="mb-3">
											<label class="form-label">{{__('Surname')}}</label>
											<input type="text" class="form-control" id="surname" name="surname" value="{{$user->surname}}" maxlength="60" onkeydown="return /[a-zA-Z]/i.test(event.key)">
										</div>
									</div>
								</div>
								...
								<div class="row">
									<div class="col-md-6">
										<div class="mb-3">
											...
											<input type="text" name="phone" id="phone" pattern="\d{10}" maxlength="10" class="form-control" data-mask-visible="true" autocomplete="off" oninput="this.value = this.value.replace(/[^0-9]/g, '')" value="{{$user->phone}}"/>
										</div>
									</div>
									<div class="col-md-6">
										<div class="mb-3">
...
<!-- 
								...
								<div class="row">
									<div class="col-md-6">
										<div class="mb-3">
											...
											<select class="form-select" id="type" name="type" onchange="getSelectedType();" disabled>
												<option value="2" {{ $selectedType == '2' ? 'selected' : '' }}>{{__('Manager')}}</option>
												<option value="3" {{ $selectedType == '3' ? 'selected' : '' }}>{{__('Contractor')}}</option>
												<!-- <option value="2" {{$user->type == 'Manager' ? 'selected' : ''}}>{{__('Manager')}}</option>
												<option value="3" {{$user->type == 'Contractor' ? 'selected' : ''}}>{{__('Contractor')}}</option> -->
											</select>
										</div>
									</div>
									<div class="col-md-6">
								...
								<button 	type="button" onclick="userSave()" form="user_edit_form" id="user_edit_button" class="btn btn-primary w-100">
									{{__('Save')}}
								</button>
							</div>
						</div>
