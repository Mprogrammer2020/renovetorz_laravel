...
@section('content')
    ...
    <div class="page-body pt-6">
        <div class="container-xl">
            <button class="btn btn-primary add-service-btn" onclick="showPropertyModal()">Add Property Type</button>
			<div class="card">
				<div id="table-default" class="card-table table-responsive">
					<table class="table table-vcenter">
						...
						@foreach($property_type_list as $key => $property_type)
							<tr>
							<td class="sort-ticketid text-capitalize">{{ $loop->iteration }}</td>

								<td class="sort-Status">{{$property_type->name}}</td>
                                <td class="sort-Status">{{$property_type->description}}</td>
								<td class="!text-start whitespace-nowrap">
										<a href="#" onclick="edit_property({{$property_type->id}},'{{$property_type->name}}','{{$property_type->description}}')" class="btn w-[36px] h-[36px] p-0 border hover:bg-[var(--tblr-primary)] hover:text-white" title="{{__('Edit')}}">
									...
									<a href="#" onclick="delete_property({{$property_type->id}})" onclick="return confirm('{{__('Are you sure? This is permanent and will delete all documents related to user.')}}')" class="btn w-[36px] h-[36px] p-0 border hover:bg-red-500 hover:text-white" title="{{__('Delete')}}">
					...
					{{ $property_type_list->links() }}
				</div>
			</div>
        </div>
    </div>

@include('panel.service.modal.property_type_modal')
@endsection
