...
@section('content')
    ...
    <div class="page-body pt-6">
        <div class="container-xl">
            <button class="btn btn-primary add-service-btn" onclick="showAddModal()">Add Service</button>
			<div class="card">
				<div id="table-default" class="card-table table-responsive">
					<table class="table table-vcenter">
						...
						@foreach($services as $key => $service)
							<tr>
								<td class="sort-ticketid text-capitalize">{{ $services->firstItem() + $loop->index }}</td>
								<td class="sort-Status">{{$service->name}}</td>
								<td class="!text-start whitespace-nowrap">
										<a href="#" onclick="edit_services({{$service->id}},'{{$service->name}}')" class="btn w-[36px] h-[36px] p-0 border hover:bg-[var(--tblr-primary)] hover:text-white" title="{{__('Edit')}}">
									...
									<a href="#" onclick="delete_services({{$service->id}})" onclick="return confirm('{{__('Are you sure? This is permanent and will delete all documents related to user.')}}')" class="btn w-[36px] h-[36px] p-0 border hover:bg-red-500 hover:text-white" title="{{__('Delete')}}">
...
@include('panel.service.modal.add_service_modal')
@endsection
