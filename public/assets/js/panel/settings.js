...
function credits_Save() {  
    // alert($("#credit_value").val());
    showLoader();
    "use strict";
    ...
    $("#testcontainer").each(function (index, form) {
    ...
    $.ajax({
        type: "post",
        url: "/dashboard/admin/settings/credit_save",
        data: formData,
        contentType: false,
        processData: false,
        success: function (data) {
...
function addMoreForm() {
    var clonedForm = $('#user_edit_form').clone();
    var uniqueId = Date.now();
    clonedForm.find(':input').val('');
    clonedForm.addClass("user_edit_form");
    clonedForm.attr("id", "user_edit_form" + uniqueId);
    clonedForm.find("[name='id']").val('');
    clonedForm.find("[name='id']").attr("id", "id_" + uniqueId);
   
    clonedForm.find("[name='leads']").attr("name", "leads_" + uniqueId);
    clonedForm.find("[name='credit_value']").attr("name", "credit_value_" + uniqueId);
    clonedForm.find("[name='amount']").attr("name", "amount_" + uniqueId);

    $('#additionalFormsContainer').append(clonedForm);

}   

function cloneForm() {
    var originalForm = document.getElementById('user_edit_form');
    ...
    document.getElementById('cloned-forms-container').appendChild(clonedForm);

    var deleteButton = document.createElement('button');
    deleteButton.type = 'button';
    deleteButton.className = 'btn btn-danger w-100 add-more-btn';
...
function submitAllForms() {
    ...
    var formData = [];
    var status = true;
    var clonedForms = document.querySelectorAll('#cloned-forms-container form');
    $('.validation-error').remove();
    clonedForms.forEach(function (clonedForm) {
        if(serializeForm(clonedForm).leads == '') {
            $(clonedForm).find('#leads').after('<div id="name-error" class="validation-error">Number of leads is required</div>');
		    status = false;
        }

        if(serializeForm(clonedForm).credit_value == '') {
            $(clonedForm).find('#credit_value').after('<div id="name-error" class="validation-error">Credit value is required</div>');
		    status = false;
        }

        if(serializeForm(clonedForm).amount == '') {
            $(clonedForm).find('#amount').after('<div id="name-error" class="validation-error">Amount is required</div>');
		    status = false;
        }
        ...
        formData.push(serializeForm(clonedForm));
    });
    if(status){
        ...
        $.ajax({
            ...
            data: JSON.stringify({ forms: formData }),
            contentType: false,
            processData: false,
            success: function (data) {
                hideLoader();
...
function deleteCredit(credit_id){
    ...
    $.ajax( {
        type: "get",
        url: "/dashboard/admin/settings/credit_delete/"+ credit_id,
        contentType: false,	
        processData: false,
        success: function ( data ) {
            showLoader();
...
function showWorkModal(){
    $('#kind_of_work').modal('show');
}

function saveKindOfWork(){
    showLoader();
    $('.validation-error').remove();

    var status = true;	
	if ($('#name').val() === '') {
		$('#name').after('<div id="name-error" class="validation-error">Name is required</div>');
		status = false;
	}

	// if ($('#description').val() === '') {
	// 	$('#description').after('<div id="name-error" class="validation-error">Description is required</div>');
	// 	status = false;
    // }
	...
	if(status == false){
		...
		$.ajax({
			type: "post",
			url: "/dashboard/admin/settings/save_KindOfWork",
			data: formData,
			contentType: false,
			processData: false,
			success: function (data) {
                $('#kind_of_work').modal('hide');
				console.log(data);
				hideLoader();
    ...
    function edit_work(id,name,description){
        // alert(id);
        $('.validation-error').remove();
    ...
    function delete_work(id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax( {
                    type: "get",
                    url: "/dashboard/admin/settings/delete_KindOfWork/"+id,
                    contentType: false,
                    processData: false,
                    success: function ( data ) {
                        console.log(data);
...
function savePropertyType(){
	...
	if(status == false){
		...
		$.ajax({
			type: "post",
			url: "/dashboard/admin/settings/save_PropertyType",
			data: formData,
			contentType: false,
			processData: false,
			success: function (data) {
                $('#typeofproperty').modal('hide');
				console.log(data);
				hideLoader();
				toastr.success('Data saved successfully.');
				location.reload();  
    ...
    function edit_property(id,name,description){
        // alert(id);
        $('.validation-error').remove();
    ...
    function delete_property(id){
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax( {
                    type: "get",
                    url: "/dashboard/admin/settings/delete_propertyType/"+id,
                    contentType: false,
                    processData: false,
                    success: function ( data ) {
                        console.log(data);
