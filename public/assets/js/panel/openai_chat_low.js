...
function updateChatButtons() {
	setTimeout( function () {
		...
		const generate = async ( ev ) => {
			...
			var gsPromptValue = promptInput.value
			promptInput.value = ''
			...
			messages.push( {
				role: "user",
				content: prompt_prefix + ' ' + gsPromptValue
			} );
			...
			try {
				...
				saveResponse( gsPromptValue, aiBubbleWrapper.querySelector( '.chat-content' ).innerHTML, chat_id )
